...
export async function generateMetadata({
  ...
  return {
    title: `${dict.bulkQuote.title} — The Unique Choice`,
    description: dict.bulkQuote.subtitle,
    alternates: localeAlternates(locale, "/bulk-quote"),
  };
}
        ...
        <section className="relative px-4 pt-28 pb-10 sm:px-6">
          <div className="mx-auto max-w-3xl text-center">
            <span className="brand-gradient mb-4 inline-block rounded-full px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-white">
              {dict.bulkQuote.badge}
            </span>
            <h1 className="font-display text-3xl font-bold tracking-tight sm:text-5xl">
              {dict.bulkQuote.title}
            </h1>
            <p className="mt-5 text-base text-muted sm:text-lg">
              {dict.bulkQuote.subtitle}
            </p>
          </div>
        </section>
